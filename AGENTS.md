...
In the template for slides do not use a bottom banner.
...
When using an article or post to create a slide, make sure the most relevant image from the post is used as illustration for that slide, resized so it does not use all the slide real estate, and use quotes from the reference when needed.
Add a slide about the speaker at the beginning with image chanezon-1-slider.png taking the full size of the slide.
When using Tweets as references you need to use Playwright instead of curl to get the content of the page.
